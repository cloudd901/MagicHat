...
namespace RandomTool
{
    public partial class Hat
    {
        ...
        {
            if (IsDisposed) { if (AllowExceptions) { throw new InvalidOperationException("This Hat has been Disposed."); } else { return -1; } }
            if (IsBusy) { if (AllowExceptions) { throw new InvalidOperationException("Hat is currently busy"); } else { return -1; } }
            if (entry.UniqueID == -1)
        ...
        {
            if (IsBusy) { if (AllowExceptions) { throw new InvalidOperationException("Hat is currently busy"); } else { return false; } }
